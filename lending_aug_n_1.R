...
for (i in 1:20){
  ...
  df_rose <- ROSE(loan_status ~., data = train)$data
...
head(opt_lambda_scad_aug_n_1)
aug_n_1_scad_result <- cbind(Accuracy,  AUC, F1, Recall,  Precision, Specificity, Sensitivity)

save(opt_lambda_scad_aug_n_1, file = 'opt_lambda_scad_aug_n_1.RData')
save(aug_n_1_scad_result, file = 'aug_n_1_scad_result.RData')

round(colMeans(aug_n_1_scad_result)*100, 2)
round(apply(aug_n_1_scad_result, 2, sd)/sqrt(20)*100 , 2)
