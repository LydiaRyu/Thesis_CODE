...
for (i in 1:20){
  ...
  n_size <- length(train$loan_status == 1) * 4
...
head(opt_lambda_scad_aug_n_4)
aug_n_4_scad_result <- cbind(Accuracy,  AUC, F1, Recall,  Precision, Specificity, Sensitivity)

save(opt_lambda_scad_aug_n_4, file = 'opt_lambda_scad_aug_n_4.RData')
save(aug_n_4_scad_result, file = 'aug_n_4_scad_result.RData')

round(colMeans(aug_n_4_scad_result)*100, 2)
round(apply(aug_n_4_scad_result, 2, sd)/sqrt(20)*100 , 2)
